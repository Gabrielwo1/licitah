...
class Acao{
    ...
    function __construct(){
        ...
        $this->quantidade = $_POST["quantidade"] ?? 1;
    }
    ...
    // Formatar o número com no máximo 2 casas decimais
    ...
    function fechaPedido(){
        ...
        $copia = "SELECT * FROM pay_carrinhos WHERE carrinho_usuario='$usuario'";
        ...
        foreach($carrinho as $id=>$quantidade){
            ...
            $item = $this->pegaInfos($produto);
            
            if($item){
                
                $produto["comprado"] = intval($quantidade);
                
                if($produto["recorrente"]){
                    $quantidade = 1;
                }
                
                if($produto["limitador"]){
                    $quantidade = 1;
                }
                ...
                if($produto["estoque"]){
                    $estoque = $produto["metas"]["estoque"] ?? 0;
                    if($estoque == 0){
                        $aut = false;
                    }else{
                ...
                if($aut){
                    ...
                    $preco = $this->paraPreco($produto["preco"]);
        ...
        $cadastra = "INSERT INTO pay_pedidos (pedido_usuario, pedido_estado,pedido_hash,pedido_autor,pedido_url, pedido_total, pedido_carrinho) VALUES ('$usuario' , '1', '$hash', '$usuario', '$url', '$calculo', '$idCarrinho')";
        if($this->conn->query($cadastra) == true){
            $id = $this->conn->insert_id;
            
            foreach($pedido as $item){
                $idProduto = $item["id"];
                $produto = json_encode($item, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
                $quantidade = $item["comprado"]; 
                $acao = "INSERT INTO pay_pedidos_itens (ppi_pedido,ppi_item,ppi_espelho, ppi_quantidade, ppi_usuario) VALUES ('$id', '$idProduto', '$produto', '$quantidade' , '$usuario')";
    ...
    function addCard(){
        if(isset($this->produto["sucesso"])){
             if($this->quantidade > 0){
                    ...
                    $usuario = $this->usuario;
                    $seleciona = "SELECT * FROM  pay_carrinhos WHERE carrinho_usuario='$usuario'";
                    $resultado = $this->conn->query($seleciona);
                    if($resultado->num_rows == 0){
                        $card = [];
                    ...
                    if($new){
                        $acao = "INSERT INTO pay_carrinhos (carrinho_usuario,carrinho_carrinho, carrinho_hash) VALUES ('$usuario', '$card', '$hash')";
                    }else{
                        $acao = "UPDATE pay_carrinhos SET carrinho_carrinho='$card' WHERE carrinho_usuario='$usuario'";
                    }
    ...
    function remover(){
       ...
       $user = $this->usuario;
       if(count($carrinho) == 0){
           $acao = "DELETE FROM pay_carrinhos WHERE carrinho_usuario='$user'";
       }else{
           $card = json_encode($carrinho);
           $acao = "UPDATE pay_carrinhos SET carrinho_carrinho='$card' WHERE carrinho_usuario='$user'";
       }
    ...
    function pegaConfigs(){
    
        $seleciona = "SELECT * FROM configuracoes_modulos WHERE config_pasta='pagamento'";
        $resultado = $this->conn->query($seleciona);
        
        $respostas = [];
        if($resultado->num_rows > 0){
            while($dado = $resultado->fetch_assoc()){
                if(!isset($respostas[$dado["config_arquivo"]])){
                    $respostas[$dado["config_arquivo"]] = [];
                }
                
                $respostas[$dado["config_arquivo"]][$dado["config_chave"]] = $dado["config_valor"];
            }
        }
        ...
        $retorno = [
            "parcelas"=>[
                "ativo" => $respostas["cartao-de-credito"]["parcelamento"] ?? false,
                "tamanho" => $respostas["cartao-de-credito"]["maximoparecelas"] ?? 1,
                "assume"=> $respostas["cartao-de-credito"]["assumeparcelas"]  ?? 1,
                "minimo"=>$resposta["cartao-de-credito"]["minimoparcelamento"] ?? false,
                ]
            ];
    ...
    function carrinhoCompleto(){
         ...
         $sql = "SHOW TABLES LIKE 'pay_carrinhos'";
         $result = $this->conn->query($sql);
    ...
    function pegaProdutos($id) {
    $lista = [];
    $seleciona = "SELECT * FROM pay_pedidos_itens WHERE ppi_pedido = ?";
    ...
    $stmt->bind_param("s", $id); // "s" indica que $id é uma string
    $stmt->execute();
    ...
    if ($resultado->num_rows > 0) {
        while ($dado = $resultado->fetch_assoc()) {
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                ...
                $lista[] = $item; // Mais curto e preferível usar o atalho []
            } else {
                // Log ou trate o erro JSON
    ...
    function pedido(){
        ...
        $produtos = $this->pegaProdutos($dado["pedido_id"]);
        $itens = [];
    ...
    function pagamentoDireto(){
        ...
        $produto = $this->pegaProduto($this->id);
        $produto = $produto["produto"];
        ...
        $calculo = number_format(floatval($calculo),2);
        $itens = json_encode($pedido);
        $cadastra = "INSERT INTO pay_pedidos (pedido_usuario, pedido_estado,pedido_hash,pedido_autor,pedido_url, pedido_total, pedido_carrinho) VALUES ('$usuario', '1', '$hash', '$usuario', '$url', '$calculo', '0')";
        if($this->conn->query($cadastra) == true){
            
             $id = $this->conn->insert_id;
             $idProduto = $produto["id"];
             $produto["comprado"] = $quantidade;
             $produto = json_encode($produto, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
            ...
            return ["erro"=>true, "mensagem"=>"Não foi possível criar o pedido"];
        }

    }
    
    function addSaldo(){
        $valor = $_POST["saldo"] ?? false;
        
        $usuario = $this->usuario;
        
        if(!$valor){
            return ["erro"=>true, "mensagem"=>"Não foi enviado um saldo válido"];
        }
        ...
        $cadastra = "INSERT INTO pay_pedidos (pedido_usuario, pedido_estado,pedido_hash,pedido_autor,pedido_url, pedido_total, pedido_carrinho, pedido_tipo) VALUES ('$usuario', '1', '$hash', '$usuario', '$url', '$calculo', '0', '1')";
        if($this->conn->query($cadastra) == true){
             return ["sucesso"=>true, "url"=>$url];
        }else{
             return ["erro"=>true, "mensagem"=>"Não foi possível criar o pedido"];
    ...
    function render(){
        ...
        switch($this->acao){
            case 'info':
                ...
                $resposta["config"] =   $this->pegaConfigs();
                return $resposta;
                break;
            case 'addSaldo':
                return $this->addSaldo();
                break;
            case 'addCard':
                if(!$this->usuario){
                    return ["erro"=>true, "mensagem"=>"Usuário não está logado"];
