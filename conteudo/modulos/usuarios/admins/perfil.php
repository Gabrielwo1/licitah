...
class Acao{
    ...
    function infos(){
        $id = $this->usuario;
        $seleciona = "SELECT * FROM usuarios WHERE usuario_hash='$id'";
        $resultado = $this->conn->query($seleciona);
    ...
    function salvaSenha(){
        if(!$this->senha){
            return ["erro"=>true, "mensagem"=>"A senha enviada não foi válida"];
        }
        
        $senha = md5(trim($this->senha));
        $usuario = $this->usuario;
        $atualiza = "UPDATE usuarios SET usuario_senha='$senha' WHERE usuario_hash='$usuario'";
        if($this->conn->query($atualiza) == true){
             return ["sucesso"=>true, "mensagem"=>"A senha foi atualizada com sucesso"];
        }else{
